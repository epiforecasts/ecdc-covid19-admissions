...
dat <- load_data(end_date = fdate) %>%
  select(id = location, date, target_variable, value)

fcast_ids <- get_forecast_ids(dat = dat,
                              forecast_date = fdate,
                              max_trunc = 30)
...
for (target in c("inc hosp", "inc case", "inc death")) {
  ...
  tsensemble_summary[[target]] <- forecast_summary(
...
tsensemble_summary <- bind_rows(tsensemble_summary)
